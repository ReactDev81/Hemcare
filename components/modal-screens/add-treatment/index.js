            ...
            <View className="flex-row justify-around mb-5 bg-white p-3 rounded-[10px]">
                ...
                <Pressable onPress={() => goToStep(3)} className="items-center gap-y-1.5" activeOpacity={1}>
                    <View className={`w-12 h-12 relative rounded-full flex items-center justify-center ${step === 3 ? 'bg-red-light' : 'bg-gray-light'}`}>
                        ...
                        <MaterialIcons name="info-outline" size={24} color={step === 3 ? '#F43F5E' : '#5C6679'} />
                    </View>
                    <Text className={`text-sm leading-4 ${step === 3 ? 'text-red-dark' : 'text-gray-dark'}`}>Info</Text>
                </Pressable>
            </View>
        );
